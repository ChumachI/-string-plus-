...
int my_sscanf(char *str, char *format, ...) {
  char *copy;
  int status = 1;
  int result = 0;
  ...
  if (my_strlen(copy) == 0 && is_invisible_action == false) {
...
int _process_f(char **str, struct Options options, va_list *args) {
  ...
  if (ret) {
    ...
    if (regexec(&regex, *str, 1, &match, 0) == 0) {
      ...
      char line[100] = {0};
      my_strncpy(line, *str + match.rm_so, size);
...
void _process_n(char *start, char *end, va_list *args) {
  ...
  if (my_strstr(start, end))
    *num = my_strlen(start) - my_strlen(end);
  else
    *num = my_strlen(start);
}
