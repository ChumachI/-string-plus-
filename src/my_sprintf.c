...
void format_float(char **buf_ptr, long double value, specifier *spec) {
  ...
  int num_str_len = my_strlen(num_str);
  ...
  if (spec->width > num_str_len + spec->plus) {
    ...
    if (spec->minus) {
      ...
      my_strcpy(*buf_ptr, num_str);
      my_memset(*buf_ptr + num_str_len, ' ', padding);
      *buf_ptr += spec->width - spec->plus;
    } else {
      my_memset(*buf_ptr, spec->zero ? '0' : ' ', padding);
      if (spec->plus) {
        *(*buf_ptr + (spec->zero ? 0 : padding)) = '+';
      }
      my_strcpy(*buf_ptr + padding + spec->plus, num_str);
      *buf_ptr += spec->width;
    }
  } else {
    if (spec->plus) {
...
void format_uint(char **buf_ptr, unsigned int value, specifier *spec) {
...
void format_g(char **buf_ptr, double value, specifier *spec) {
  ...
  if (padding > 0) {
    my_memmove(start_ptr + padding, start_ptr, written);
    my_memset(start_ptr, ' ', padding);
    *buf_ptr += padding;
  }
}
